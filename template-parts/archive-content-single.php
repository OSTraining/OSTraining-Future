...
<article class="post">
	...
	if ( has_post_thumbnail() ) {
		...
		$image = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'ostraining-future-large' );

		// if the image is wider than tall then print it
		if ( $image[1] > $image[2] ) {
			echo '<a href="' . esc_url( get_permalink() ) . '" class="image featured">';
			the_post_thumbnail( 'ostraining-future-large' );
			echo '</a>';
		}
	}
	?>
	...
	<footer>
		<ul class="actions">
			<li><a href="<?php esc_url( the_permalink() ); ?>" class="button big"><?php _e( 'Continue Reading', 'ostraining-future' ); ?></a></li>
		</ul>
		<ul class="stats">
