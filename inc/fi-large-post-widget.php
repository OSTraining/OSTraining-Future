...
class Future_Imperfect_Large_Post_List_Widget extends WP_Widget {
	...
	function __construct() {
		parent::__construct(
			...
			__( 'Future Imperfect Large Post List', 'ostraining-future' ), // Name
			array( 'description' => __( 'Lists posts with a large icon', 'ostraining-future' ), ) // Args
		);
	}
		...
		if ( $the_query->have_posts() ) {
			...
			while ( $the_query->have_posts() ) {
				...
				if ( has_post_thumbnail() ) {
					$thumbnail = wp_get_attachment_image_src( get_post_thumbnail_id( get_the_ID() ), 'ostraining-future-small' );

					$output .= '<a href="' . esc_url( get_permalink() ) . '" class="image">';
					//$output .= get_the_post_thumbnail( get_the_ID(), 'ostraining-future-small' );
